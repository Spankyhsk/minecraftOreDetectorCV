# -*- coding: utf-8 -*-
"""
Erz-spezifische Kandidaten- und Sonderfallfilter.
"""
...
class CoalDetector:
    ...
    def find_candidates(self, img: np.ndarray, coal_mask: np.ndarray) -> List[Box]:
        ...
        coal_seed = cv2.morphologyEx(coal_seed, cv2.MORPH_OPEN, np.ones((3, 3), np.uint8))
        grouped = cv2.dilate(coal_seed, np.ones((9, 9), np.uint8), iterations=2)
        grouped = cv2.morphologyEx(grouped, cv2.MORPH_CLOSE, np.ones((11, 11), np.uint8))
